...
function authorized(req){const p=process.env.ADMIN_PASSWORD;if(!p)return false;const supplied=(req.headers.authorization||'').replace(/^Bearer /,'');return timingSafeEqual(createHash('sha256').update(p).digest(),createHash('sha256').update(supplied).digest());}
